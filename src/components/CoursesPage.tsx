...
export const CoursesPage: React.FC<CoursesPageProps> = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 mb-24">
      {/* Header Banner */}
      <div className="neu-card p-6 mb-6 border border-amber-400/30 bg-gradient-to-r from-[#121E36] via-[#182848] to-[#0E172C] relative overflow-hidden">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <span className="text-xs font-bold px-3 py-1 bg-amber-500/20 text-amber-300 border border-amber-500/40 rounded-full inline-flex items-center gap-1.5 mb-2">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap cursor-pointer ${
              selectedCategory === cat.id
                ? 'bg-[#0E1A30] border border-amber-400/70 text-amber-300 shadow-[inset_2px_2px_5px_#060a17]'
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {filteredCourses.map((course) => (
          <div
            key={course.id}
            className="neu-card p-5 flex flex-col justify-between hover:border-amber-400/50 transition-all group"
          >
            <div>
              <div className="flex items-start justify-between gap-2 mb-3">
                <span className="text-[10px] font-black px-2.5 py-1 bg-amber-500/20 text-amber-300 border border-amber-500/40 rounded-full">
                  {course.badge}
                </span>
              ...
              {/* Topics List */}
              <div className="mt-4 space-y-2 bg-[#0D172A] p-3 rounded-xl border border-slate-700/50">
            ...
            <div className="mt-5 pt-3 border-t border-slate-700/50 flex items-center justify-between gap-2">
              <span className="text-xs font-semibold text-slate-400">
                {course.enrolledCount}
              </span>
              <button
                onClick={() => setActiveCourseModal(course)}
                className="neu-btn px-4 py-2 rounded-xl text-xs font-extrabold text-amber-300 hover:text-amber-200 flex items-center gap-1 cursor-pointer"
              >
                <span>ভিউ কোর্স</span>
      ...
      {activeCourseModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="neu-card max-w-lg w-full p-6 relative border border-amber-400/50">
            <h3 className="text-xl font-black text-white mb-2">{activeCourseModal.title}</h3>
            <p className="text-xs text-slate-300 mb-4">{activeCourseModal.subtitle}</p>

            <div className="space-y-2 bg-[#0C1527] p-4 rounded-xl border border-slate-700/60 mb-5 text-xs text-slate-200">
              <p className="font-bold text-amber-300 mb-2">কোর্স আউটলাইন & কন্টেন্ট:</p>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="neu-btn flex-1 py-3 rounded-xl text-xs font-black text-amber-300 flex items-center justify-center gap-2 cursor-pointer"
              >
              ...
              <button
                ...
                className="px-4 py-3 rounded-xl text-xs font-bold bg-slate-800 text-slate-300 hover:text-white cursor-pointer"
              >
                বন্ধ করুন
              </button>
            </div>
