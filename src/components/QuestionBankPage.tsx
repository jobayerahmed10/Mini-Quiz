...
export const QuestionBankPage: React.FC<QuestionBankPageProps> = ({
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 pb-24 space-y-6 animate-fade-in">
      ...
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-[#0B132B] via-[#121E36] to-[#0B132B] rounded-3xl p-6 text-white shadow-md relative overflow-hidden">
      ...
      <div className="space-y-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 bg-white border border-[#E6E2D3] rounded-2xl text-xs sm:text-sm text-[#0B132B] font-semibold focus:outline-none focus:border-[#0B132B] shadow-xs"
          />
        </div>
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-extrabold whitespace-nowrap transition-all cursor-pointer ${
                selectedType === t.id
                  ...
                  : 'bg-white text-slate-600 border border-[#E6E2D3] hover:bg-slate-100'
              }`}
            >
              {t.label}
            </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {filteredPapers.map((paper) => (
          <div
            key={paper.id}
            className="bg-white rounded-2xl border border-[#E6E2D3] p-5 shadow-xs hover:border-[#0B132B] hover:shadow-md transition-all flex flex-col justify-between space-y-4"
          >
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-bold px-2.5 py-0.5 bg-blue-100 text-[#0B132B] rounded-full">
                  {paper.examType}
                </span>
              </div>

              <h2 className="text-sm sm:text-base font-black text-[#0B132B] leading-snug">
                {paper.title}
              </h2>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-[11px] text-slate-600 font-medium bg-slate-50 p-3 rounded-xl">
              <div>
                ...
                <p className="font-bold text-[#0B132B]">{paper.totalQuestions}টি</p>
              </div>
              <div>
                <p className="text-slate-400">সময়</p>
                <p className="font-bold text-[#0B132B]">{paper.timeMinutes} মিনিট</p>
              </div>
              <div>
                <p className="text-slate-400">পাশ মার্কস</p>
                <p className="font-bold text-[#0B132B]">{paper.passingMarks}%</p>
              </div>
            </div>

            <button
              onClick={() => onStartPastPaperTest(paper)}
              className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-[#0B132B] hover:bg-[#121E36] text-white text-xs font-bold rounded-xl transition-all shadow-xs cursor-pointer"
            >
